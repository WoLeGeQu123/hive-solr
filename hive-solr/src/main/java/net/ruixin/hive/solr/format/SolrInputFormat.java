...
public class SolrInputFormat extends
    ...
    public RecordReader<LongWritable, MapWritable> getRecordReader(
        ...
        String hiveColumnNameProperty = conf.get(serdeConstants.LIST_COLUMNS);
...
//            throw new IOException("no column mapping found!");
            if(StringUtils.isBlank(hiveColumnNameProperty)){
                throw new IOException("no column mapping found!");
            }
            columns = hiveColumnNameProperty.split(",");
        } else {
            columns = ConfigurationUtil.getAllColumns(columnString);
        }
        ...
        return new SolrReader(ConfigurationUtil.getUrl(conf), ConfigurationUtil.isCloud(conf),
    ...
    public InputSplit[] getSplits(JobConf conf, int numSplits)
            throws IOException {
        return SolrSplit.getSplits(conf, ConfigurationUtil.getUrl(conf),
                ConfigurationUtil.isCloud(conf),
                ConfigurationUtil.getCollectionName(conf),
                numSplits);
    }
